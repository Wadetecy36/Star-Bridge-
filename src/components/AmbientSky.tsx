...
export default function AmbientSky({ reducedMotion = false }: { reducedMotion?: boolean }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const stars: Star[] = [];
      const resize = () => { dpr = Math.min(window.devicePixelRatio || 1, 1.5); width = Math.max(1, window.innerWidth); height = Math.max(1, window.innerHeight); canvas.width = Math.floor(width * dpr); canvas.height = Math.floor(height * dpr); context.setTransform(dpr, 0, 0, dpr, 0, 0); stars.length = 0; const count = Math.min(120, Math.max(46, Math.floor(width * height / 10000))); for (let i = 0; i < count; i += 1) { const depth = i % 10 < 2 ? 1.5 : i % 3 === 0 ? 1 : .55; stars.push({ x: Math.random() * width, y: Math.random() * height, size: (.35 + Math.random() * 1.25) * depth, phase: Math.random() * Math.PI * 2, speed: (.0017 + Math.random() * .006) * depth, depth }); } };
      const pointer = (event: PointerEvent) => { targetX = (event.clientX / width - .5) * 14; targetY = (event.clientY / height - .5) * 10; };
      const frame = (time: number) => { if (paused) return; const delta = Math.min(48, time - (last || time)); last = time; pointerX += (targetX - pointerX) * .025; pointerY += (targetY - pointerY) * .025; context.clearRect(0, 0, width, height); for (const star of stars) { if (!reducedMotion) { star.y += star.speed * delta; if (star.y > height + 5) star.y = -5; } const alpha = .18 + ((Math.sin(time / (850 + star.depth * 400) + star.phase) + 1) * .5) * (.56 * star.depth / 1.5); context.beginPath(); context.fillStyle = `rgba(255,248,237,${alpha})`; context.arc(star.x + pointerX * star.depth, star.y + pointerY * star.depth, star.size, 0, Math.PI * 2); context.fill(); }
      ...
      cleanup = () => { cancelAnimationFrame(animation); window.removeEventListener('resize', resize); window.removeEventListener('pointermove', pointer); document.removeEventListener('visibilitychange', visibility); };
    } catch { /* decorative atmosphere must never break the app */ }
    return cleanup;
  }, [reducedMotion]);
