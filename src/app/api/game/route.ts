...
export async function POST(request: NextRequest) {
  ...
  if (action === 'new_round') {
    const difficulty = String(body?.difficulty || 'normal'); const stored = await supabaseAdmin.from('user_settings').select('*').eq('user_id', me.id).maybeSingle();
  ...
  if (action === 'click_star') {
    ...
    if (!star?.is_target || star.locked_at) return NextResponse.json({ ok: true, completed: false });
    const { data: people } = await supabaseAdmin.from('users').select('id').eq('room_id', me.room_id).order('created_at'); const first = people?.[0]?.id; const field = first === me.id ? 'clicked_by_user_1' : 'clicked_by_user_2';
    const update: Record<string, string> = { [field]: me.id }; const partnerAlreadyClicked = field === 'clicked_by_user_1' ? star.clicked_by_user_2 : star.clicked_by_user_1;
    if (partnerAlreadyClicked) update.locked_at = new Date().toISOString();
    const { error: updateError } = await supabaseAdmin.from('constellation_stars').update(update).eq('id', star.id); if (updateError) return NextResponse.json({ error: updateError.message }, { status: 400 });
    let completed = false;
    if (partnerAlreadyClicked) {
      const { count } = await supabaseAdmin.from('constellation_stars').select('id', { count: 'exact', head: true }).eq('round_id', round.id).eq('is_target', true).not('locked_at', 'is', null);
      if ((count || 0) >= round.target_count) {
        const completedAt = new Date().toISOString(); const { data: completedRound } = await supabaseAdmin.from('constellation_rounds').update({ status: 'completed', completed_at: completedAt }).eq('id', round.id).eq('status', 'active').select().maybeSingle();
        completed = Boolean(completedRound); if (completed) await supabaseAdmin.from('audit_log').insert({ user_id: me.id, event_type: 'constellation_completed', metadata: { difficulty: round.difficulty, stars: round.target_count } });
      }
    }
